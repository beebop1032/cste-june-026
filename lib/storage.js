...
// Direct overwrite — no archiving, one Blob PUT call
export async function write(name, data) {
  await writeRaw(name, data)
}

// Archiving write — keeps a versioned backup before overwriting (used for prof submissions)
...
async function blobRead(name) {
  ...
  if (!blob) return null
  // Use the full URL from list() so get() doesn't need to extract storeId from token
